...
for iter=1:n_iter
    ...
    for A=1:NA
        ...
        cost(A)=itae(Nodes(ant(A,1)),Nodes(ant(A,2)),Nodes(ant(A,3)),Nodes(ant(A,4)),Nodes(ant(A,5)));
...
%if plotfig
 %   figure(3)
  %  plot(t,reference,t,output)
   % end
Kp=Nodes(ant(cost_best_ind),1)
Ki=Nodes(ant(cost_best_ind),2)
Kd=Nodes(ant(cost_best_ind),3)
lam=Nodes(ant(cost_best_ind),4)
mu=Nodes(ant(cost_best_ind),5)
