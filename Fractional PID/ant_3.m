...
LB=[0.01 0.01 0.01 0.01 0.01]; % lower bound
% UB=10.*ones(1,27); % upper bound
UB=[10 5 5 1 1]; % upper bound
...
for iter=1:n_iter
    ...
    for A=1:NA
        ...
        disp(['Ant Paramters: ' num2str(Nodes(ant(A,1),1))])
        if iter~=1
    ...
    if (cost_best>cost_best_prev) && (iter~=1)
        ...
        x_best_prev=cost_best;
        best_prev_ant=ant(cost_best_ind,:)
    end
    
    
    
    dT=zeros(n_node,n_param); % Change of Pheromone
    for tour_i=1:n_param
        for A=1:NA
...
Kp=Nodes(ant(cost_best_ind,1),1)
Ki=Nodes(ant(cost_best_ind,2),2)
Kd=Nodes(ant(cost_best_ind,3),3)
lam=Nodes(ant(cost_best_ind,4),4)
mu=Nodes(ant(cost_best_ind,5),5)
